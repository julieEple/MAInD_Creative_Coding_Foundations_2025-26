...
function showSnake(){
    ...
    for (let i = 0; i < snake.length; i++) { //go through every part of the snake (every cell element)
      ...
      // Draw the apples using Pokeberries generated in getRandomBerry
        for (let i = 0; i < apples.length; i++) { //loop through every apple-elememnt (should be 3)
          let apple = apples[i];
          let r = apple.rc;
          let c = apple.cc;
      
          // getRandomBerry returns a random berry that can be used as apple
          getRandomBerry().then(berrySprite => {
            if (berrySprite) {
              cells[r][c].innerHTML = `<img src="${berrySprite}">`; // using the API-img of the berry inside the cell-element that marks the apple
            } else {
              // just in case soemhting happens to the API
      ...
      function getRandomBerry() { //this is where the berry-magic happens. Made by copying the lecture of weather-API logic as best as I can
        return fetch('https://pokeapi.co/api/v2/berry/') //a list of 20 different berries
          ...
          .then(response => response.json()) //one more URL to collect 
          .then(itemData => {
