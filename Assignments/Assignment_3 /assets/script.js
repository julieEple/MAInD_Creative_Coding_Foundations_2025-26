...
function makeApple() {
  while (apples.length < amountOfApples) {
    ...
    if (!onSnake) {
        const newApple = { rc: randR, cc: randC, icon: "🍎" }; //temporary apple-icon until API is loaded 
        apples.push(newApple);

        getRandomBerry().then((icon) => { //updates the icon of the apple with a icon generated by my getRandomBerry-function
            if (icon) newApple.icon = icon;
          });
    }
  }
}
